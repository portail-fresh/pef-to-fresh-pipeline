...
def dispatch_data_access(xml_file: str, input_folder: str, output_folder: str, context=None):
    """
    Applies field-level transformations to the given XML file using an Excel mapping
    specific to data access fields, and logs changes using a changelog system.
    ...
    try:
        ...
        logger.info("Starting data access transformation for file: %s", xml_file)

        input_path = Path(input_folder) / xml_file
        output_path = Path(output_folder) / xml_file

        task_name = "dispatch_data_access"
        ...
        excel_path = "dispatch-data-access-fr-en.xlsx"
        ...
        logger.info("Successfully applied data access transformation and saved: %s", output_path)
